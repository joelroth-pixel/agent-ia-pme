...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .alert { display: flex; align-items: center; gap: 12px; padding: 12px; border-radius: 8px; margin-bottom: 8px; }
    .alert.urgence { background: #fef2f2; border-left: 4px solid #ef4444; }
    .alert.prospect { background: #f0fdf4; border-left: 4px solid #22c55e; }
    .alert .icon { font-size: 20px; }
    .alert .content .title { font-weight: 600; font-size: 14px; }
    .alert .content .desc { font-size: 12px; color: #64748b; margin-top: 2px; }
    ...
    .notif-status { font-size: 12px; color: #22c55e; margin-top: 4px; display: none; }
  </style>
</head>
<body>
...
<div id="dashboard" style="display:none">
  ...
  <div class="container">
    ...
    <div class="card">
      ...
      <div id="alerts-list">
        <div class="empty">Aucune alerte cette semaine</div>
      </div>
    </div>
  ...
  function loadData() {
    ...
    .then(data => {
      ...
      if (data.prospects && data.prospects.length > 0) {
        alertsList.innerHTML = data.prospects.map(p =>
          '<div class="alert prospect"><div class="icon">👤</div><div class="content"><div class="title">' + p.name + '</div><div class="desc">' + p.phone + '</div></div></div>'
        ).join('');
      } else {
        alertsList.innerHTML = '<div class="empty">Aucune alerte cette semaine</div>';
      }
